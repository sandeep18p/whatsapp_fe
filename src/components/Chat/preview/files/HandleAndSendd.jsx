...
import { removeFileFromFiles, sendMessage } from "../../../../feature/chatSlice";
import SocketContext from "../../../../context/SocketContext";
import VideoThumbnail from "react-video-thumbnail";

function HandleAndSendd({ activeIndex, setActiveIndex, message,socket}) {
    ...
    setLoading(true);
     ...
     //[ayload has all the information that is there]
     setLoading(false);
    }
     //Handle remove file
    ...
    return (
        <div className="w-[97%] flex items-center justify-between mt-2 border-t dark:border-dark_border_2">
            ...
            <div className="flex gap-x-2">
                {/* Map over files */}
                {files.map((file, i) => (
                    <div key={i} className={`fileThumbnail relative w-14 h-14 border dark:border-white mt-2 rounded-md overflow-hidden cursor-pointer ${
...
const HandleAndSendWithContext = (props) => (
    <SocketContext.Consumer>
      {(socket) => <HandleAndSendd {...props} socket={socket} />}
    </SocketContext.Consumer>
  );
